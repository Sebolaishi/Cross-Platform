...
public class RestResponseExceptionHandler extends ResponseEntityExceptionHandler implements ExceptionMessages {
    ...
    protected ResponseEntity<Object> handleConflict( Exception exception, WebRequest request) {
        CrossPlatformError error = CrossPlatformError.of(HttpStatus.NOT_FOUND.name(), DIRECTORY_NOT_FOUND);
    ...
    protected ResponseEntity<Object> notFoundExceptions( Exception exception, WebRequest request) {
        CrossPlatformError error = CrossPlatformError.of(HttpStatus.NOT_FOUND.name(), NOT_FOUND);
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
    ...
    protected ResponseEntity<Object> noContentExceptions( Exception exception, WebRequest request) {
        CrossPlatformError error = CrossPlatformError.of(HttpStatus.NO_CONTENT.name(), NO_CONTENT);
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
